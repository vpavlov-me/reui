    ...
    <header className={cn("sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60", className)}>
      <div className="container max-w-[1048px] flex h-14 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map((item) =>
              item.items ? (
                ...
                <Button key={item.label} variant="ghost" size="sm" className="h-8 px-3" asChild>
                  <Link href={item.href || "#"}>
                    {item.icon && <item.icon className="mr-2 h-4 w-4" />}
                    {item.label}
                  </Link>
        ...
        <div className="flex items-center gap-2">
          {showNotifications && (
            <Button variant="ghost" size="icon" className="h-10 w-10" onClick={onNotificationClick}>
