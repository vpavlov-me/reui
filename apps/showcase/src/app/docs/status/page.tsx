...
function StatusBadge({ status }: { status: ComponentStatus }) {
  const variants: Record<ComponentStatus, { variant: "default" | "secondary" | "destructive" | "outline"; icon: React.ReactNode }> = {
    stable: { variant: "default", icon: <Check className="h-2.5 w-2.5 md:h-3 md:w-3 mr-1" /> },
    beta: { variant: "secondary", icon: <Clock className="h-2.5 w-2.5 md:h-3 md:w-3 mr-1" /> },
    experimental: { variant: "outline", icon: <TestTube className="h-2.5 w-2.5 md:h-3 md:w-3 mr-1" /> },
    deprecated: { variant: "destructive", icon: <AlertTriangle className="h-2.5 w-2.5 md:h-3 md:w-3 mr-1" /> },
  };
  ...
  return (
    <div className="space-y-6 md:space-y-10">
      <div className="space-y-3 md:space-y-4">
        <h1 className="text-2xl md:text-3xl font-medium tracking-tight">Component Status</h1>
      ...
      <section className="space-y-3 md:space-y-4">
        <h2 id="summary" className="text-xl md:text-2xl font-semibold tracking-tight scroll-mt-20">
          Summary
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4">
          <div className="rounded-lg border p-3 md:p-4">
            <div className="text-xl md:text-2xl font-medium">{totalComponents}</div>
            <div className="text-xs md:text-sm text-muted-foreground">Total</div>
          </div>
          <div className="rounded-lg border p-3 md:p-4">
            <div className="text-xl md:text-2xl font-medium text-green-600">{stableComponents}</div>
            <div className="text-xs md:text-sm text-muted-foreground">Stable</div>
          </div>
          <div className="rounded-lg border p-3 md:p-4">
            <div className="text-xl md:text-2xl font-medium text-yellow-600">{betaComponents}</div>
            <div className="text-xs md:text-sm text-muted-foreground">Beta</div>
          </div>
          <div className="rounded-lg border p-3 md:p-4">
            <div className="text-xl md:text-2xl font-medium text-blue-600">{documentedComponents}</div>
            <div className="text-xs md:text-sm text-muted-foreground">Docs</div>
          </div>
          <div className="rounded-lg border p-3 md:p-4">
            <div className="text-xl md:text-2xl font-medium text-purple-600">{testedComponents}</div>
            <div className="text-xs md:text-sm text-muted-foreground">Tests</div>
          </div>
        </div>
      </section>
      ...
      {/* Status Legend */}
      <section className="space-y-3 md:space-y-4">
        <h2 id="legend" className="text-xl md:text-2xl font-semibold tracking-tight scroll-mt-20">
          Status Legend
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          <div className="flex items-start gap-2 md:gap-3 p-3 md:p-4 rounded-lg border">
            ...
            <div className="min-w-0">
              <div className="font-medium text-sm md:text-base">Stable</div>
          ...
          <div className="flex items-start gap-2 md:gap-3 p-3 md:p-4 rounded-lg border">
            ...
            <div className="min-w-0">
              <div className="font-medium text-sm md:text-base">Beta</div>
              <div className="text-xs md:text-sm text-muted-foreground">
          ...
          <div className="flex items-start gap-2 md:gap-3 p-3 md:p-4 rounded-lg border">
            ...
            <div className="min-w-0">
              <div className="font-medium text-sm md:text-base">Experimental</div>
              <div className="text-xs md:text-sm text-muted-foreground">
          ...
          <div className="flex items-start gap-2 md:gap-3 p-3 md:p-4 rounded-lg border">
            ...
            <div className="min-w-0">
              <div className="font-medium text-sm md:text-base">Deprecated</div>
              <div className="text-xs md:text-sm text-muted-foreground">
      ...
      {categories.map((category) => (
        <section key={category} className="space-y-3 md:space-y-4">
          <h2 id={category.toLowerCase().replace(/\s+/g, '-')} className="text-xl md:text-2xl font-semibold tracking-tight scroll-mt-20">
            {category}
          </h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    <th className="p-2 md:p-3 text-left font-medium whitespace-nowrap">Component</th>
                    <th className="p-2 md:p-3 text-left font-medium whitespace-nowrap">Status</th>
                    <th className="p-2 md:p-3 text-left font-medium whitespace-nowrap hidden sm:table-cell">Version</th>
                    <th className="p-2 md:p-3 text-left font-medium whitespace-nowrap">Docs</th>
                    <th className="p-2 md:p-3 text-left font-medium whitespace-nowrap hidden md:table-cell">Tests</th>
                  </tr>
                </thead>
                <tbody>
                  {components
                    ...
                    .map((component) => (
                      <tr key={component.name} className="border-b last:border-0">
                        <td className="p-2 md:p-3 font-medium whitespace-nowrap">{component.name}</td>
                        <td className="p-2 md:p-3">
                          <StatusBadge status={component.status} />
                        </td>
                        <td className="p-2 md:p-3 font-mono text-xs whitespace-nowrap hidden sm:table-cell">{component.version}</td>
                        <td className="p-2 md:p-3">
                          {component.docsPath ? (
                            <Link
      ...
      {/* Changelog Preview */}
      <section className="space-y-3 md:space-y-4">
        <h2 id="changelog" className="text-xl md:text-2xl font-semibold tracking-tight scroll-mt-20">
          Recent Changes
        </h2>
        <div className="space-y-3 md:space-y-4">
          <div className="rounded-lg border p-3 md:p-4">
            <div className="flex flex-wrap items-center gap-2 mb-2 md:mb-3">
              <Badge variant="outline" className="">v0.1.0</Badge>
              <span className="text-xs md:text-sm text-muted-foreground">Initial Release</span>
            </div>
